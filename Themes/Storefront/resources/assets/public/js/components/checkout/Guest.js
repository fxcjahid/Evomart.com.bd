...
export default {
	...
	created() {
		...
		this.$nextTick(() => {
			...
			if (window.Stripe) {
				this.stripe = window.Stripe(Evomart.stripePublishableKey);
	...
	methods: {
		...
		confirmFlutterWavePayment({
			...
			FlutterwaveCheckout({
				...
				customizations: {
					title: Evomart.storeName,
					logo: Evomart.storeLogo,
				},
				onclose(incomplete) {
					vm.placingOrder = false;
