		...
		<div class="container">
			<div class="tab-products-header clearfix">
				<ul class="tabs float-left">
					<li v-for="(tab, index) in tabs" :key="index" :class="classes(tab)"
						@click="change(tab)">
						{{ tab.label }}
					</li>
				</ul>
			...
			<dynamic-tab v-for="(tabLabel, index) in data" :key="index" :label="tabLabel" :url="route('storefront.tab_products.index', {
				sectionNumber: 1,
				tabNumber: index + 1,
			})
				">
