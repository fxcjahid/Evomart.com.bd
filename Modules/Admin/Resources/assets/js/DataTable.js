...
Evomart.dataTable = { routes: {}, selected: {} };
...
export default class {
	constructor(selector, options, callback) {
		...
		if (Evomart.dataTable.selected[selector] === undefined) {
			Evomart.dataTable.selected[selector] = [];
		}
	...
	appendToSelected(id) {
		...
		if (!Evomart.dataTable.selected[this.selector].includes(id)) {
			Evomart.dataTable.selected[this.selector].push(id);
		}
	}
	...
	removeFromSelected(id) {
		...
		Evomart.dataTable.selected[this.selector].remove(id);
	}
	...
	route(name, params) {
		let router = Evomart.dataTable.routes[this.selector][name];
	...
	hasRoute(name) {
		return Evomart.dataTable.routes[this.selector][name] !== undefined;
	}

	static setRoutes(selector, routes) {
		Evomart.dataTable.routes[selector] = routes;
	}

	static setSelectedIds(selector, selected) {
		Evomart.dataTable.selected[selector] = selected;
	}

	static getSelectedIds(selector) {
		return Evomart.dataTable.selected[selector];
	}
