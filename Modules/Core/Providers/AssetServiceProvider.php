...
use Modules\Core\Foundation\Asset\Manager\EvomartAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\AssetPipeline;
use Modules\Core\Foundation\Asset\Pipeline\EvomartAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, EvomartAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new EvomartAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (!is_null($assets = config("evomart.themes.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("evomart.modules.{$module->get('alias')}.assets");
