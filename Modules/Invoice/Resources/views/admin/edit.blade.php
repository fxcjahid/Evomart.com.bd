...
@component('admin::components.page.header')
    @slot('title', 'Edit Invoice - ' . $orders->id) 
@endcomponent

@section('content')
    <div class="box box-primary" id="create-invoice"> 
        <update-form :orders="{{ $orders }}" :gateways="{{ $gateways }}" :shippingmethods="{{ $shippingMethod }}" inline-template>
    ...
    <script>  
        ...
        Vue.component('updateForm', {
            props: [ 'orders', 'gateways', 'shippingmethods', 'options'], 
            data() {  
				let selectedKey = [];
				...
				products.forEach(element => {
					selectedKey.push(element.product_id)
				});
                ...
                return{  
					...
					btnLoading: false, 
                    laodingCart: true,    
					productPrice: orders.sub_total.amount, 
                    productList: {},   
					errors: new Errors(),
                    form:{ 
						selected_products: selectedKey, 
						invoice_number: orders.id,
						customer_name: orders.customer_name,
						customer_phone: orders.customer_phone,
						customer_address: orders.billing_address_1,
						courier_name: orders.courier_name,
						shipping_city: orders.shipping_city,
						order_note: orders.note,
						payment_method: this.makeUnderscores(orders.payment_method), 
                        shipping_method: this.makeUnderscores(orders.shipping_method),   
						shipping_cost: orders.shipping_cost.amount,
						discount: orders.discount.amount,
            ...
            methods:{
                ...
                onChangeProducts(){
                    ...
                    document.addEventListener("DOMContentLoaded", function() { 
                        document.querySelector('.selectize-input > input').addEventListener('blur', ()=>{
                            const dataSelectable = document.querySelectorAll('#selectedProducts > option');
                ...
                getSelectedProducts(){  
                    ...
                    $.ajax({
                        ...
                        data: { productsID: selectedProducts },
                    }).then((response) => {  
